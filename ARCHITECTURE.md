...
the compiler. On the headline benchmark this cut **703,374 tokens to
55,625 (12.6×)** for the same task at better answer quality — and it
makes serious coding work viable on a free, fully offline local model.
...
**Stage 1 — Index.** Every Rust and TypeScript source (the lane is
chosen per file extension; adding a language adds a lane, not an
architecture) is taken apart at symbol granularity: each function, type, and method is extracted
into the embedded **k-lani WORM/ACID engine** (write-once append-only
tables, transactional WAL-first commits) together with its
documentation, signature, and a name-based dependency graph. A full
...
|---|---|---|
| Architecture question, ~700-file workspace | 703,374 tokens / 41 requests | **55,625 tokens / 10 requests (12.6×)** |
...
| Test coverage of the harness itself (lines, llvm-cov) | — | 92.9 % (171 tests, every feature test-first) |

**The harness holds itself to its own standard.** Every feature in
this system was written test-first against its own gate; the suite
covers 92.9 % of all lines. Coverage proves diligence, not
correctness — correctness is what the deterministic guards and the
immutable ledgers are for — but it tells you how this thing was
built.
