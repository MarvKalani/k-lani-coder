...
## Account pools (several API-key contingents)

The vault label is the account identity. Add two or more entries with
the **same provider and model pattern** but distinct labels:

```bash
printf '%s' "$OPENROUTER_TEAM_A_KEY" | k-lani-ai-proxy vault add \
  --provider openrouter --model "openrouter/*" --label team-a \
  --data-dir ./data/proxy

printf '%s' "$OPENROUTER_TEAM_B_KEY" | k-lani-ai-proxy vault add \
  --provider openrouter --model "openrouter/*" --label team-b \
  --data-dir ./data/proxy
```

The proxy round-robins the accounts in deterministic label order. On
HTTP 429 it tries each remaining account in that pool at most once
before returning the final 429. It does **not** fail over on semantic
or authentication errors such as 400/401, and it never retries
forever. Adding a new key with the same provider, pattern, and label
replaces only that account; an unlabeled entry keeps the legacy
single-account behavior.

Every skipped 429 attempt and terminal proxy-log line carries
`account=<alias>`; skipped attempts also append an `upstream_retry`
HTTP trace. The live Models view therefore shows sprint usage,
availability, and measured rolling limit windows per account
contingent. Labels are account aliases and must contain only ASCII
letters, digits, `-`, and `_`.

Account pools are intended for sanctioned **pay-per-token API keys**,
where parallel calls and multiple project or billing keys are allowed
by the provider. Pooling consumer subscription accounts to multiply or
circumvent personal limits can violate provider terms and risk account
suspension. That use is an owner decision and responsibility, not a
capability the proxy endorses.
...
- Auth passthrough bypasses vault credentials and account pools; the
  inbound caller owns the credential used for that request.
